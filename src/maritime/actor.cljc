(ns maritime.actor
  "MaritimeDeckActor — the ISCO-08 3152 ship deck coordination
  actor as a `langgraph.graph/state-graph` (ADR-2607011000). One graph run =
  one maritime deck operation (intake → advise → govern → decide →
  ...
  This actor coordinates voyage planning, position reporting, navigational
  hazard flagging, and port-arrival logistics. It does NOT make actual
  navigation decisions, command course changes, resolve collision-avoidance
  scenarios, or exercise command authority (those remain the ship's exclusive
  human authority at sea).
  ...
  The unconditional invariant: the MaritimeDeckAdvisor can never directly commit
  a record the MaritimeDeckGovernor refuses — every commit-record! call
...
(defn build-graph
  "Build a compiled MaritimeDeckActor graph. `store` implements
  `maritime.store/Store`. `advisor` implements `maritime.advisor/Advisor`
